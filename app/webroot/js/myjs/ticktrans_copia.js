$(document).ready(function(){
	...
	$('#guardar_cliente').on('click', function(event){

		var nombre = $("#nombrecliente").val();
   		var url = "../../clientes/index";

   		guardarCliente(nombre, url);
	...
	function guardar_datos(event){
		...
		var datos = {
			...
			'monto'				: $("#monto").val(),
			'transcriptor_id'	: $("#transcriptor").val(),
		...
		$.ajax({
			...
			type : 'POST',
			url: basePath + 'detalleTickets/add',
			data: datos,
			dataType : 'json',
				encode : true,
	...
	function cambio_servicio(cantidad, servicio){
		...
		$.ajax({
			...
			success: function(data){
					...
					if (cantidad != '') {
						$('.precio').val('Bs.' + data.mostrar_datos.precio);
	...
	function guardarCliente(nombre, url)
	{
   			...
   			if (nombre == '')
   			{
   				...
   				data: {nombre: nombre},
	   				...
	   				error: function(){
				      alert('Error en función javascript de guardarCliente de ticktrans.js');
				    }
